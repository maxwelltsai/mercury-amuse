...
! Retrieve the potential at a particle position
...
! Retrieve the acceleration vector of a particle. Second time derivative of the
! position.
function get_acceleration(id, ax, ay, az) result(ret)
      use amuse_mercuryMod
      implicit none
      integer :: ret, id
      real*8, intent(out) :: ax, ay, az
      !ret = amuse_get_acceleration(id, ax, ay, az)
end function

! set user-defined force (Maxwell)
function set_user_defined_force(ax,ay,az,n) result(ret)
      use amuse_mercuryMod
      implicit none
      integer :: n, ret
      real*8, intent(in) :: ax(n), ay(n), az(n)
      ret = amuse_set_user_defined_force(ax,ay,az,n)
end function

! set perturbers (Maxwell)
function set_perturbers(m_p, x_p, y_p, z_p, vx_p, vy_p, vz_p, ax_p, ay_p, az_p, jx_p, jy_p, jz_p, n) result(ret)
      use amuse_mercuryMod
      implicit none
      integer :: n, ret
      ...
      ret = amuse_set_perturbers(m_p, x_p, y_p, z_p, vx_p, vy_p, vz_p, ax_p, ay_p, az_p, jx_p, jy_p, jz_p, n)
end function
