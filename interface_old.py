...
class Mercury(MercuryWayWard):
    ...
    def get_acceleration(self, id):
        if id<0 or id>len(self.particles)-1:
            raise Exception('Invalid particle ID')
        x = self.particles[id].x
        y = self.particles[id].y
        z = self.particles[id].z
        eps2 = self.get_eps2()
        return self.get_gravity_at_point(eps2,x,y,z)

    def set_acceleration(self, id, ax, ay, az):
        raise Exception('Mercury does not support setting acceleration')

    def get_number_of_particles(self):
        return len(self.particles)

    def get_potential(self, id):
        if id<0 or id>len(self.particles)-1:
            raise Exception('Invalid particle ID')
        x = self.particles[id].x
        y = self.particles[id].y
        z = self.particles[id].z
        eps2 = self.get_eps2()
        return self.get_potential_at_point(eps2,x,y,z)

    def set_state(self, id, mass, x, y, z, vx, vy, vz, radius):
        if id<0 or id>len(self.particles)-1:
            raise Exception('Invalid particle ID')
        self.particles[id].x = x
        self.particles[id].y = y
        self.particles[id].z = z
        self.particles[id].vx = vx
        self.particles[id].vy = vy
        self.particles[id].vz = vz
        self.particles[id].mass = mass
        self.particles[id].radius = radius

    def get_state(self, id):
        if id<0 or id>len(self.particles)-1:
            raise Exception('Invalid particle ID')
        p = self.particles[id]
        return p.mass, p.x, p.y, p.z, p.vx, p.vy, p.vy, p.vz, p.radius
    ...
    def get_gravity_at_point(self,radius,x,y,z):
        eps2=self.parameters.epsilon_squared
        ax = 0
        ay = 0
        az = 0
        for p in self.particles:
            ...
            if xx==x and yy==y and zz==z:
                # Exclude the particle's self gravity if the queried position is at the particle
                continue

            dr2=((xx-x)**2+(yy-y)**2+(zz-z)**2+eps2)

            ax+=constants.G*p.mass*(xx-x)/dr2**1.5
            ay+=constants.G*p.mass*(yy-y)/dr2**1.5
            az+=constants.G*p.mass*(zz-z)/dr2**1.5
